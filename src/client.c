 ...
 */
int get_server_message(packet_list *pkt_list, int sockfd)
{
...
int decode_message(packet_list *pkt_list)
{
...
/*!
 * \brief Adiciona espacos nos campos vazios do final do pacote 
 * 
 * \param[out] pkt Pacote
 * \param[in] re resto da divisao do tamanho do pacote pelo tamanho
 * da mensagem.
 * 
 */

static void add_spaces(packet *pkt, int re)
{
  int i;
  for (i = re; i <= PACKET_MSG_SIZE; i++)
    pkt->decoded_packet[i] = SPACE;
}

/*!
 * \brief Funcao para checar se e necessario adicionar espacos 
 * no final do pacote 
 * 
 * \param[out] pkt Pacote
 * \param[in] message Mensagem decodificada
 * 
 */

static void check_msg_size_and_copy(packet *pkt, char *message)
{
  int msg_size = 0, re = 0;
  msg_size = strlen(message);
  if (!(re = (msg_size % PACKET_MSG_SIZE)))
    memcpy(pkt->decoded_packet, message, PACKET_MSG_SIZE);
  else
  {  
    memcpy(pkt->decoded_packet, message, re);
    add_spaces(pkt, re);
  }  
}
/*!
...
{  
  ...
  {    
    if (pkt->next == NULL)
      check_msg_size_and_copy(pkt, message);
    else  
      memcpy(pkt->decoded_packet, message, PACKET_MSG_SIZE);
    encode_values(pkt);
    build_packet(pkt);  
...
void encode_message(packet *pkt, char *message)
{   
  trim_spaces(message);
...
int send_message(packet *pkt, int sockfd)
{  
  while (pkt != NULL)
  {
...
int receive_confirmation(packet_list *pkt_list, int sockfd)
{ 
  int ret = 0;
  pkt_list->head = NULL; 
